...
/obj/item/stock_parts/power_store/attack_self(mob/user)
	...
	var/obj/item/organ/internal/stomach/ethereal/user_stomach = human_user.get_organ_slot(ORGAN_SLOT_STOMACH)
...
// MONKE EDIT: ETHEREAL DRAIN REPLACED WITH MONKE VERSION ENTIRELY
...
/obj/item/stock_parts/power_store/proc/ethereal_drain(mob/living/carbon/human/user, obj/item/organ/internal/stomach/ethereal/used_stomach)
	...
	if(charge < CELL_POWER_DRAIN)
		...
		return
	if(user.blood_volume > charge_limit)
		to_chat(user, span_warning("Your charge is full!"))
		return
	to_chat(user, span_notice("You begin clumsily channeling power from [src] into your body."))
	used_stomach.drain_time = world.time + CELL_DRAIN_TIME
	if(!do_after(user, CELL_DRAIN_TIME, target = src))
		return
	if((charge < CELL_POWER_DRAIN) || (user.blood_volume > charge_limit))
		return
	if(!istype(used_stomach))
		to_chat(user, span_warning("You can't receive charge from [src]!"))
		return
	to_chat(user, span_notice("You receive some charge from [src], wasting some in the process."))
	used_stomach.adjust_charge(CELL_POWER_GAIN)
