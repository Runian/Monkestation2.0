...
/datum/antagonist/highlander/proc/give_equipment()
	...
	for(var/obj/item/I in H)
		if(!H.dropItemToGround(I))
			qdel(I)
	H.regenerate_icons()
	H.revive(ADMIN_HEAL_ALL, revival_policy = POLICY_ANTAGONISTIC_REVIVAL)
...
/datum/antagonist/highlander/robot/give_equipment()
	...
	if(!istype(robotlander))
		return ..()
	robotlander.revive(ADMIN_HEAL_ALL, revival_policy = POLICY_ANTAGONISTIC_REVIVAL)
	...
	robotlander.apply_model(/obj/item/robot_model/syndicate/kiltborg)
	robotlander.apply_skin(robotlander.model.default_skin)
